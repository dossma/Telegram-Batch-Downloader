...
def write_meta(file_fullpath, meta_dict):
    ...
    try:  # Z.B. AVI-Dateien erzeugen Fehler und Abbruch
        ...
        print(f"Metadata cannot be written for {file_fullpath!s}: {e}")
        return 
    if mutag is None:
        print("Format of metadata not supported. \nMetadata won't be written")
        return
    ...
    #             audio.save()
        ...
        raise TypeError(f"Writing metadata for {type(mutag)} not implemented")
        # print(f"Metadaten‑Schreiben für {type(mutag)} nicht implementiert")
    
    # FÜR DEBUGGING:
    print(f"- Metadata written -") # in\n'{file_fullpath}'")
